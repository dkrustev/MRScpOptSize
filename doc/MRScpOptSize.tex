...
\else
  ...
  %\providecommand{\publicationstatus}{} % do not show publication status
...
\else
\def\titlerunning{Controlling Size Of Supercompiled Programs Using MRSC}
\fi
...
\else
      \Comment{$P$ -- program (function definitions); }
      \State \Comment{$l$ -- nesting level; }
      \State \Comment{$h$ -- history (list of tuples: local/global flag; level; configuration); }
      \State \Comment{$c$ -- configuration}
\fi      
...
\ifVptVer
The extended version of this article \cite{krustev2020MrscOptSizeReport} discusses a few additional examples as well.
\else
